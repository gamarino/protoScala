/*
 * UnicodeLetters — the non-ASCII code points the lexer accepts in alphanumeric identifiers
 * (Lexer::identCharLength): Unicode letters (general categories
 * Lu, Ll, Lt, Lm, Lo), combining marks (Mn, Mc, Me), so that a letter written
 * with a combining accent stays inside its identifier, and decimal digits (Nd).
 * ASCII characters are classified by the lexer itself.
 *
 * GENERATED from the Unicode Character Database 16.0.0 (Python's unicodedata
 * module); do not edit by hand. Each entry is a maximal run of consecutive
 * code points >= U+0080 selected by
 *
 *     cat = unicodedata.category(chr(cp))
 *     cat[0] in "LM" or cat == "Nd"
 *
 * in ascending order.
 */
#pragma once
...
inline constexpr CodePointRange kSymbolLetterRanges[] = {
    ...
    {0x2E2F, 0x2E2F}, {0x3005, 0x3006}, {0x302A, 0x302F}, {0x3031, 0x3035},
    {0x303B, 0x303C}, {0x3041, 0x3096}, {0x3099, 0x309A}, {0x309D, 0x309F},
    ...
    {0xA674, 0xA67D}, {0xA67F, 0xA6E5}, {0xA6F0, 0xA6F1}, {0xA717, 0xA71F},
    ...
    {0x102E0, 0x102E0}, {0x10300, 0x1031F}, {0x1032D, 0x10340},
    {0x10342, 0x10349}, {0x10350, 0x1037A}, {0x10380, 0x1039D},
    ...
    {0x10A80, 0x10A9C}, {0x10AC0, 0x10AC7}, {0x10AC9, 0x10AE6},
    {0x10B00, 0x10B35}, {0x10B40, 0x10B55}, {0x10B60, 0x10B72},
    {0x10B80, 0x10B91}, {0x10C00, 0x10C48}, {0x10C80, 0x10CB2},
    {0x10CC0, 0x10CF2}, {0x10D00, 0x10D27}, {0x10D30, 0x10D39},
    {0x10D40, 0x10D65}, {0x10D69, 0x10D6D}, {0x10D6F, 0x10D85},
    {0x10E80, 0x10EA9}, {0x10EAB, 0x10EAC}, {0x10EB0, 0x10EB1},
    {0x10EC2, 0x10EC4}, {0x10EFC, 0x10F1C}, {0x10F27, 0x10F27},
    ...
    {0x113C7, 0x113CA}, {0x113CC, 0x113D3}, {0x113E1, 0x113E2},
    ...
    {0x11680, 0x116B8}, {0x116C0, 0x116C9}, {0x116D0, 0x116E3},
    {0x11700, 0x1171A}, {0x1171D, 0x1172B}, {0x11730, 0x11739},
    {0x11740, 0x11746}, {0x11800, 0x1183A}, {0x118A0, 0x118E9},
    {0x118FF, 0x11906}, {0x11909, 0x11909}, {0x1190C, 0x11913},
    {0x11915, 0x11916}, {0x11918, 0x11935}, {0x11937, 0x11938},
    {0x1193B, 0x11943}, {0x11950, 0x11959}, {0x119A0, 0x119A7},
    ...
    {0x11C38, 0x11C40}, {0x11C50, 0x11C59}, {0x11C72, 0x11C8F},
    ...
    {0x11F3E, 0x11F42}, {0x11F50, 0x11F5A}, {0x11FB0, 0x11FB0},
    {0x12000, 0x12399}, {0x12480, 0x12543}, {0x12F90, 0x12FF0},
    {0x13000, 0x1342F}, {0x13440, 0x13455}, {0x13460, 0x143FA},
    {0x14400, 0x14646}, {0x16100, 0x16139}, {0x16800, 0x16A38},
    {0x16A40, 0x16A5E}, {0x16A60, 0x16A69}, {0x16A70, 0x16ABE},
    {0x16AC0, 0x16AC9}, {0x16AD0, 0x16AED}, {0x16AF0, 0x16AF4},
    {0x16B00, 0x16B36}, {0x16B40, 0x16B43}, {0x16B50, 0x16B59},
    {0x16B63, 0x16B77}, {0x16B7D, 0x16B8F}, {0x16D40, 0x16D6C},
    {0x16D70, 0x16D79}, {0x16E40, 0x16E7F}, {0x16F00, 0x16F4A},
    ...
    {0x1E290, 0x1E2AE}, {0x1E2C0, 0x1E2F9}, {0x1E4D0, 0x1E4F9},
    {0x1E5D0, 0x1E5FA}, {0x1E7E0, 0x1E7E6}, {0x1E7E8, 0x1E7EB},
...
// True when the non-ASCII code point `cp` is a letter, a combining mark or a
// decimal digit. Binary search over kSymbolLetterRanges (781 ranges), at most
// 10 probes.
inline bool isSymbolLetter(char32_t cp) {
    std::size_t lo = 0;
    std::size_t hi = std::size(kSymbolLetterRanges);
    while (lo < hi) {
        const std::size_t mid = lo + (hi - lo) / 2;
        if (cp > kSymbolLetterRanges[mid].last) {
            lo = mid + 1;
        } else if (cp < kSymbolLetterRanges[mid].first) {
            hi = mid;
        } else {
            return true;
        }
