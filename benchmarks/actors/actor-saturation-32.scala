// Mode `saturation-32`: 32 actors x N/8 CPU-bound messages, one sender thread.
//
...
// With 32 actors there are always more runnable actors than workers, so the
// pool can steal freely and no worker should idle waiting for a busy actor.
// Read against `saturation-8`, this separates "the scheduler cannot fill the
// cores" from "8 actors cannot fill the cores". This mirrors protoST's
// `saturation_32a.st`. Total work is identical to `saturation-8` (same N, same
// per-message compute), so the two curves are directly comparable.
//
...
println("mode=saturation-32 messages=" + messages.toString + " processed=" + processed.toString)
println(Actor.stats)
