...
| Id | Deviation | Track |
|---|---|---|
| D1 | `Int`/`Long` promote to arbitrary precision instead of wrapping; integer literals are not range-checked either: `0xFFFFFFFF` is `4294967295` (Scala: `-1`), and `2147483648` or `99999999999999999999` are accepted without `L` (Scala: a compile error). **There is therefore no integer width**, and three things follow from that rather than from overflow: a **shift count is not masked** (D112), **`>>>` of a negative operand is refused** (D15), and `toInt`/`toLong` do not truncate (D29) | (perm) |
...
| D15 | `>>>` of a **negative** operand raises `UnsupportedOperationException`: the vacated bits come from the operand's width, and Scala's own answer differs by width (`-8 >>> 1` is `2147483644` as an `Int` and `9223372036854775804` as a `Long`), so protoScala refuses rather than returning one of the two silently. For a **non-negative** operand there are no bits to fill, so `>>>` is the arithmetic shift and agrees with Scala exactly — the `(lo + hi) >>> 1` idiom works. Corrected by Track S; it used to raise unconditionally | Q12 |
...
| Id | Deviation | Where |
|---|---|---|
| D19 | A negative (or overflowing) shift amount to `<<`/`>>`/`>>>` raises `IllegalArgumentException` rather than being masked to a word width (consistent with D1: no fixed integer width; see D112 for a non-negative count) | `Primitives.cpp::shiftAmount` |
...
| D31 | Methods cannot be overloaded **in a template** (a second definition of a name there is an error); constructors may be overloaded by number of parameters only. Top-level `def`s are overloadable by number of parameters as of Track S — see D111 | Q11 |
...
| D109 | **A `type` alias is recorded unit-wide, under its simple name.** Scala scopes a type member to its template, so `class A { type T = Int }` and `class B { type T = String }` in one file are two different `T`s; protoScala keeps one table for the unit and the later declaration wins. Nothing else about aliases diverges -- `type X = T`, `type X[A] = T`, the abstract forms `type X` and `type X >: L <: U`, chains of aliases, and an alias used as a parent, in a `new`, in a type pattern or in an `isInstanceOf` all behave as scalac's do. Honouring the scope needs a type-name scope stack the compiler does not have, and an alias is a naming concern only because types are erased (D4). Fixture: `tests/conformance/29-type-aliases/` | Track S | later |
| D110 | **Type-directed widening reaches as far as the declared type is written, and no further.** Scala widens an integer to the expected floating-point type; protoScala has no expected type (D4), so it widens wherever `Double` or `Float` is written **at the point of declaration** -- a `val`, a `var`'s initialiser, a `def` result, a `def`, lambda, constructor or case-class parameter, a class field and an explicit `(e: Double)` ascription all give scalac's answer (`val d: Double = 42` is `42.0`). Two sites do not, and print the integer: an **assignment** to a variable declared earlier (`var v: Double = 1; v = 2` is `2`, scalac `2.0`, including `p.x = 3` on a `var` field), and a **type argument** (`val l: List[Double] = List(4, 5)` is `List(4, 5)`, scalac `List(4.0, 5.0)`), which is erased. The assignment case was left alone deliberately: matching it means remembering which names were declared `Double`, and without a type-name scope a name matched across scopes would turn a correct `2` into a wrong `2.0` -- a worse defect than the one it fixed. Fixture: `tests/conformance/02-expressions/double-widening{,-remainder}.scala` | Track S | later |
