...
| D25 | Top-level duplicate definitions are allowed — redefining a top-level `val`/`var`/`def` replaces the earlier binding, matching REPL redefinition semantics; a duplicate name inside a block is still a compile error | `Compiler.cpp::compileUnit` vs. `compileBlock` |
