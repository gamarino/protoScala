...
| 2026-09-23 | D35–D40 added to the provisional Phase 2 list; they were collected while implementing the phase and answer no numbered question: a refutable generator without `case` filters (D35), for-comprehension desugaring differences that leave the result unchanged (D36), intersection types cannot be tested at run time (D37), the default `toString` shape (D38), `List` hash codes (D39), the `override`-on-`var` diagnostic wording (D40) | agent, pending review | docs/STATUS.md, docs/LANGUAGE.md §5 |
